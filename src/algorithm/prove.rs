...
impl InnerProof {
    ...
    pub fn new(
        ...
        let inner_proof_content = G1Affine::sum_of_products(&prover_params.generators_alpha[prover_params.n-index..2*prover_params.n-index], &scalars_u64);
    ...
    // Updating an existing inner proof.
    pub fn update_inner_proof(
        &mut self,
        prover_params: &ProverParams,
        ...
        if proof_index.0!=changed_index.0{
            return Err(String::from("Only inner proof in the same row needs updating"));
        }
        ...
        if proof_index.1 != changed_index.1 {
            ...
            self.inner_proof_content.add_assign(&res);
        }
    ...
    // Part 1 of aggregate_inner_proof: Aggregate the proofs in the same inner commitment
    pub fn aggregate_inner_proofs_same_inner_commitment(
        commit: &G1,
        proofs: &[Self],
        set: &[usize],
    ...
    )-> Result<Self, String>
    {
        ...
        let mut bases: Vec<G1> = proofs.iter().map(|s| s.inner_proof_content).collect();
        ...
        let inner_proof_content = G1Affine::sum_of_products(&bases_affine[..], &scalars_u64);

        Ok(InnerProof{inner_proof_content})
    }
    // Part 2 of aggregate_inner_proof: cross-commitment aggregate the already aggregated proofs
    pub fn aggregate_aggregated_inner_proofs_from_different_inner_commitments(
        commits: &[G1],
        proofs: &[Self],
        set: &[Vec<usize>],
    ...
    pub fn aggregate_inner_proof(
        ...
        let mut bases: Vec<G1> = proofs.concat().iter().map(|x| x.inner_proof_content).collect();
        CurveProjective::batch_normalization(&mut bases);
    ...
    pub fn verify_single_inner_proof(
        &self,
        ...
        let mut proof_mut = self.inner_proof_content;
        proof_mut.negate();
    ...
    pub fn batch_verify_inner_proof_same_commitment(
        &self,
        verifier_params: &VerifierParams,
        com: &G1,
        set: &[usize],
        ...
        if set.len() == 1 {
            return self.verify_single_inner_proof(&verifier_params, &com, value_sub_vector[0], set[0]);
        }
        // 1. compute tmp
    ...
    pub fn batch_verify_inner_proof(
        &self,
        verifier_params: &VerifierParams,
        com: &[G1],
        set: &[Vec<usize>],
        ...
        let mut tmp2 = self.inner_proof_content;
        tmp2.negate();
...
impl OuterProof {
    ...
    pub fn new(
        ...
        let outer_proof_content =  G1Affine::sum_of_products(&new_beta_alpha_power, &scalars_u64);
    ...
    pub fn update_outer_proof(
        ...
        if proof_index != changed_index.0 {
            ...
            self.outer_proof_content.add_assign(&res);
        }
    ...
    pub fn aggregate_outer_proof(
        ...
        let mut bases: Vec<G1> = proofs.iter().map(|s| s.outer_proof_content).collect();
        CurveProjective::batch_normalization(&mut bases);
        let bases_affine: Vec<G1Affine> =
            bases.iter().map(|s| s.into_affine()).collect();
        // proof = \prod proofs[i]^ti[i]
        let outer_proof_content = G1Affine::sum_of_products(&bases_affine[..], &scalars_u64);

        Ok(OuterProof{outer_proof_content})
    }

    // Verify the single outer proof.
    pub fn verify_single_outer_proof(
        &self,
        verifier_params: &VerifierParams,
        com: &G1,
        ...
        let rhs = Bls12::pairing_product(
            self.outer_proof_content.into_affine(),
            G2Affine::one(),
    ...
    pub fn batch_verify_outer_proof(
        &self,
        verifier_params: &VerifierParams,
        com: &G1,
        set: &[usize],
        ...
        // if the length == 1, call normal verification method
        if set.len() == 1 {
            return self.verify_single_outer_proof(&verifier_params, com, &inner_commitment_sub_vector[0], set[0]);
        }
        // 1. compute t_i
        // 1.1 get the list of scalas, return false if this failed
        ...
        g1_vec.push(self.outer_proof_content.into_affine());
