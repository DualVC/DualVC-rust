mod algorithm;
use  algorithm::basic::Commitment;
use  algorithm::basic::InnerProof;
use  algorithm::basic::OuterProof;
...
fn test_all_alg()
{
        ...
        let mut inner_proofs: Vec<InnerProof> = vec![];
        let mut outer_proofs: Vec<OuterProof> = vec![];
    
        let start = time::now();//获取开始时间  
        for i in 0..n {
            for j in 0..n{ 
                inner_proofs.push(InnerProof::new(&pp, &values[i*n..(i+1)*n], j).unwrap());
            }
        }
        let end = time::now();//获取结束时间
        println!("Single ProveInner:{:?}",(end-start)/(n*n) as i32);
        ...
        let end = time::now();//获取结束时间
        println!("Single ProveOuter:{:?}",(end-start)/n as i32);
        ...
        for i in 0..n {
            for j in 0..n{
                let index=i*n+j;
                let res=inner_proofs[index].verify_single_inner_proof(&vp, &com.commit_c1[i], values[index], j);
                if !res {
                    println!("({},{})inner proof verify res is {}",i, j, res);
                }
                
            }
        }
        let end = time::now();//获取结束时间
        println!("Single Verify Inner Proof:{:?}",(end-start)/(n*n) as i32);
        ...
        let end = time::now();//获取结束时间
        println!("Single Verify Outer Proof:{:?}",(end-start)/n as i32);
        ...
        let mut in_sub_proof: Vec<Vec<InnerProof>> = vec![];//{\pi_ij}(i, j)\in S
        ...
        for i in 0..outer_size {
            ...
            let mut line_in_sub_proof: Vec<InnerProof> = vec![];
            for j in 0..inner_size {
                line_in_set.push(j);
                line_in_sub_value.push(values[i*n+j].clone());
                line_in_sub_proof.push(inner_proofs[i*n+j].clone());
            }
            ...
            out_set.push(i);
            out_sub_value.push(com.commit_c1[i]);
            out_sub_proof.push(outer_proofs[i].clone());
        }
       
        //2. aggregate proofs
        let start = time::now();//获取开始时间  
        let agg_in_pf=InnerProof::aggregate_inner_proof(&out_sub_value, &in_sub_proof, &in_set, &in_sub_value, n).unwrap();
        let end = time::now();//获取结束时间
        println!("Aggregate {} inner Proofs:{:?}",outer_size*inner_size, end-start);
        
        let start = time::now();//获取开始时间  
        let agg_out_pf=OuterProof::aggregate_outer_proof(&com.commit_c2, &out_sub_proof, &out_set, &out_sub_value, n). unwrap();
        let end = time::now();//获取结束时间
        println!("Aggregate {} outer Proofs:{:?}",outer_size, end-start);
    
        /*batch verification*/
        let start = time::now();//获取开始时间  
        let res = agg_in_pf.batch_verify_inner_proof(&vp, &out_sub_value, &in_set, &in_sub_value);
        if !res{
            println!("The verification of agg_in_pf is {}", res);
        }
        let end = time::now();//获取结束时间
        println!("Batch verify the aggregated inner proof:{:?}",end-start);
    
        let start = time::now();//获取开始时间  
        let res = agg_out_pf.batch_verify_outer_proof(&vp, &com.commit_c2, &out_set, &out_sub_value);
        if !res{
            println!("The verification of agg_out_pf is {}", res);
        }
        let end = time::now();//获取结束时间
        println!("Batch verify the aggregated outer proof:{:?}",end-start);
        ...
        //2. update commit
        let start = time::now();//获取开始时间   
        com.update_inner_commitment(&pp, changed_index, delta_value).unwrap();
        let end = time::now();//获取结束时间
        println!("Update inner commitment:{:?}",end-start);
    
        let start = time::now();//获取开始时间   
        com.update_outer_commitment(&pp, changed_index, delta_value).unwrap();
        let end = time::now();//获取结束时间
        println!("Update outer commitment:{:?}",end-start);
        //3. update inner proof in the same line
        let start = time::now();//获取开始时间   
        for j in 0..n{
            inner_proofs[changed_index.0*n+j].update_inner_proof(&pp, (changed_index.0, j), changed_index, delta_value).unwrap();
        }
        let end = time::now();//获取结束时间
        println!("Update an inner proof:{:?}",(end-start)/n as i32);
        println!("Update all related inner proofs:{:?}",end-start);
    
        //4. update outer proof
        let start = time::now();//获取开始时间   
        for i in 0..n{
            outer_proofs[i].update_outer_proof(&pp, i, changed_index, delta_value).unwrap();
        }
        let end = time::now();//获取结束时间
        println!("Update an outer proof:{:?}",(end-start)/n as i32);
        println!("Update all related outer proofs:{:?}",end-start);
        ...
        for i in 0..n {
            for j in 0..n{
                ...
                let res=inner_proofs[index].verify_single_inner_proof(&vp, &com.commit_c1[i], values[index], j);
                if !res {
                    println!("({},{})inner proof verfy res is {}",i, j, res);
                }
